...
download_release() {
	...
	"darwin")
		...
		"x86_64")
			url="$GH_REPO/releases/download/v${version}/${TOOL_NAME}-${arch}-apple-darwin.tar.xz"
			;;
		esac
		;;
	...
	"linux")
		...
		"armv7")
			url="$GH_REPO/releases/download/v${version}/${TOOL_NAME}-${arch}-unknown-linux-musleabi.tar.xz"
			;;

		"riscv64gc")
			url="$GH_REPO/releases/download/v${version}/${TOOL_NAME}-${arch}-unknown-linux-gnu.tar.xz"
			;;

		"x86_64")
			url="$GH_REPO/releases/download/v${version}/${TOOL_NAME}-${arch}-unknown-linux-musl.tar.xz"
			;;
		esac
		;;
	esac
...
install_version() {
	...
	(
		...
		ln -s "$install_path/$tool_cmd" "$install_path/$TOOL_NAME"
